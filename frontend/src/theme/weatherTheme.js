...
const VAR_MAP = {
  ...
  // The caution band needs two values, and only in light appearance. #F9A825 is
  // the specified warning colour and it is right for a fill, a dot or a stroke
  // — but at 1.97:1 on white it is unreadable as text, so anything that renders
  // the band as *words* uses `--color-caution-ink` instead. On a dark ground the
  // amber is already readable and the two tokens hold the same value.
  '--color-caution-ink': 'cautionInk',
